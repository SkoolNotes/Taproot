...
* $\int \sin^2 x\cos^2 x dx$
  ...
  \text{And the derivative:}
  \[\begin{aligned}
  \frac{d}{dx} \sin  x \cos  x &= - \sin  ^2 x + \cos  ^2 x \\
  &= \cos  ^2 x - \sin  ^2 x
  \end{aligned}\]

  \text{Now we can use differentiation by parts on $(\sin  x \cos  x) (\sin  x \cos  x)$:}
  ...
  \text{To evaluate the integral of $\sin  ^4 x$, we can use differentiation by parts after multiplying by one.}
  \text{We will need to use the integral of $\sin  x \cos  x$ from earlier in this problem, as well as the integral of $\sin  ^2 x$ from the previous problem.}
  ...
  \text{Plugging back into the main problem:}
