...
* motivation
  If we have some vector $b$ which is not in the column space of $A$ (there does not exist $x : Ax = b$) but we still want the best "approximation", then we want to take the "closest" approximation
