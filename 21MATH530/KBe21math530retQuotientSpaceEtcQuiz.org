...
** $ST = I$ iff $TS = I$
   ...
   \[ TS(s_i) = T(t_i) = s_i \]
   Because the identity map also takes each $s_i \to s_i$, and linear maps are uniquely defined by where they take their bases, $TS$ must be the identity map. The argument is symetric for the only if direction.
