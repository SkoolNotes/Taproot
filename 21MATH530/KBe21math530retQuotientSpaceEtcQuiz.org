...
** $ST = I$ iff $TS = I$
   ...
   \[ ST(t_i) = S(s_i) = t_i \]
   Because the identity map also takes each $t_i \to t_i$, and linear maps are uniquely defined by where they take their bases, $TS$ must be the identity map. The argument is symetric for the only if direction.
