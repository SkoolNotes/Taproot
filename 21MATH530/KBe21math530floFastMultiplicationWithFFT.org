...
*** now to multiply, we can just take enough points on each polynomial and multiply those points to get $n_1 + n_2 + 1$ points on the product polynomial, and then solve for the actual equation
*** only O(d) operations
...
**** now what initial points do we want to choose... an example shows
