...
* Forward Direction
  ...
  This argument does not work for infinite dimensional vector spaces. Instead, we can send anything not in $U$ to where $T$ would send it and show that the resulting $S$ is linear.
  Let $S : V/U \to W$ s.t. $S(\pi v) = Tv$.
