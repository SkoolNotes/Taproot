...
** In the reverse direction
   Intuitively: if any information is lost, then some of it must be lost to zero because zero is an element in every vector space and information should be lost "linearly" meaning relatively evenly.
   Given that $\text{null }T = 0$ and supposing we had $u, v \in V$ such that $Tu = Tv$,
