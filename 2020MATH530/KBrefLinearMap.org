...
** $\mathbb{F}^n \to \mathbb{F}^m$
   ...
   Notice that this is equivalent to taking $A$ as a $n\times m$ matrix and dot producting it with the $m \times 1$ matrix $\begin{bmatrix}x_1 \\ x_2 \\ \vdots \\ x_m\end{bmatrix}$.
